...
export const CartDrawer = ({ isOpen, onClose }: CartDrawerProps) => {
    ...
    return (
        <>
            ...
            <div className={`${isOpen ? "bg-black/70 visible" : "bg-transparent invisible"} fixed top-0 bottom-0 left-0 right-0 z-40`}
                onClick={onClose}>
                <div className={`${isOpen ? "translate-x-0" : "translate-x-full"} absolute top-0 right-0 bottom-0 bg-white pt-6 transition-all duration-500 ease-in-out w-75 md:w-106 z-50`} onClick={(e) => e.stopPropagation()}>
                    <header className="flex items-center justify-between px-5">
                        <p className="text-2xl">Carrinho ({cart.length})</p>
                        <button className="text-xl cursor-pointer" onClick={onClose}>
                            X
                        </button>
                    </header>

                    <ul className="px-4 overflow-y-auto scrollbar-hide h-[calc(100%_-_140px)] flex flex-col gap-3">
                        {cart.map(product => (
                    ...
                    <footer className="absolute bottom-0 w-full h-25 p-4">
                        <button className="w-full h-full bg-black hover:bg-gray-800 text-white rounded-xs cursor-pointer">
                            Fechar pedido
                        </button>
                    </footer>
                </div>
